...
class Model:
        ...
        if self.output_dir is not None:
            ...
            flog.write(str(sys.argv[0])+"\n"
                    +"the filer number of conv1:"+ str(self.dimension1)+"\n"
                      ...
                      +"the filer number of conv2:"+ str(self.dimension20)+"\n"
                      +"the filer size of conv2:"+ str(self.conv20_filter)+"\n"
    ...
    def prediction(self):
        ...
        h_pool1 = max_pool_2x2(tf.nn.dropout(tf.nn.relu(h_conv11), self.keep_prob))
        ...
        h_pool2 = max_pool_2x2(tf.nn.dropout(h_conv2, self.keep_prob))
        ...
        """h_conv2_rc = tf.nn.relu(conv2d_1(h_pool1_rc, tf.reverse(W_conv2, [0,1])))
        h_pool2_rc = tf.nn.dropout(max_pool_2x2(h_conv2_rc), self.keep_prob)
        
        h_conv21_rc = tf.nn.dropout(tf.nn.relu(conv2d_1(h_pool2_rc, tf.reverse(W_conv21, [0,1]))), self.keep_prob2)
        h_pool21_rc = max_pool_2x2(h_conv21_rc)

        h_conv22_rc = tf.nn.dropout(tf.nn.relu(conv2d_1(h_pool21_rc, tf.reverse(W_conv22, [0,1]))), self.keep_prob2)
        h_pool22_rc = max_pool_4x1(h_conv22_rc)    """    
        ...
        """
        W_fc1_rc = weight_variable([1 * self.fc1_param * self.dimension22, self.dimension4], 'W_fc1_rc')
        l2norm_list.append(tf.reduce_sum(tf.square(W_fc1_rc)))
        W_fc1_rc.assign(tf.cond(l2norm_list[6]>cond, lambda: tf.multiply(W_fc1_rc, cond/l2norm_list[6]),lambda: W_fc1_rc ))
        b_fc1_rc = bias_variable([self.dimension4], 'b_fc1_rc')
        l2norm_list.append(tf.reduce_sum(tf.square(b_fc1_rc)))
        b_fc1_rc.assign(tf.cond(l2norm_list[7]>cond, lambda: tf.multiply(b_fc1_rc, cond/l2norm_list[7]),lambda: b_fc1_rc ))"""

        
        h_pool3_flat = tf.reshape(h_pool22, [-1, 1*self.fc1_param*self.dimension22])
        #h_pool3_flat_rc = tf.reshape(h_pool22_rc, [-1, 1*self.fc1_param*self.dimension22])
        #h_pool3_flat_conc=tf.concat([h_pool3_flat,h_pool3_flat_rc], 1)
        #print tf.shape(h_pool3_flat_conc)
        #h_fc1 = tf.add(tf.nn.relu(tf.add(tf.matmul(h_pool3_flat, W_fc1), b_fc1)),tf.nn.relu(tf.add(tf.matmul(h_pool3_flat_rc, W_fc1_rc), b_fc1_rc)))
        ...
        variable_dict={"W_conv1": W_conv1, "W_conv2": W_conv2,"W_conv21": W_conv21, "W_conv22": W_conv22, 
                        "W_fc1": W_fc1,
                        #"W_fc1_rc": W_fc1_rc,
                        "W_fc4": W_fc4, 
                        "b_fc1": b_fc1,
                        #"b_fc1_rc": b_fc1_rc, 
                        "b_fc4": b_fc4}
        neurons_dict={"h_conv21":h_conv21, "h_conv2":h_conv2, "h_conv11":h_conv11,"h_fc1_drop": h_fc1_drop}
