...
class Model(object):
    ...
    def prediction(self):
        ...
        W_conv1_rc = weight_variable([self.conv1_filter, 4, 1, self.dimension1], 'W_conv1_rc')
        cond=tf.constant(0.9)
        wconv1_rc_l2=tf.reduce_sum(tf.square(W_conv1_rc))
        l2norm_list.append(wconv1_rc_l2)
        W_conv1_rc.assign(tf.cond(wconv1_rc_l2>cond, lambda: tf.multiply(W_conv1_rc, cond/wconv1_rc_l2),lambda: W_conv1_rc ))
