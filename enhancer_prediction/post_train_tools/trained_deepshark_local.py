...
import network_constructors.network_constructor2 as nc
...
def process(file):
    ...
    try:
        ...
        sys.exit(0)
    try:
        genome_seq_re_list, chromosome, chr_position=genome_scan(file[0])
    except:
    ...
    for j in range(y_len):
        ...
        for i in range(y_len_j):
            value=y_prediction1[j][i][0]-y_prediction1[j][i][1]
            if value>0.0:
    ...
    sess.close()
    out=open(file[1]+str(chromosome)+"_srt.bed", 'w')
...
def main():
    ...
    loopnumber=int(math.ceil(float(len(f))/float(CPUnum)))
    ...
    for i in range(loopnumber):
        if i==loopnumber-1:
            ...
            task=f[s:(s+CPUnum)]
        print task
        #p = multiprocessing.Pool(processes=CPUnum) 
        for t in task:
            t_=[t, out_dir]
            try:
                process(t_)
