...
y_ = tf.placeholder(tf.float32, shape=[None, 93])
...
h_fc1_drop=tf.matmul((y-current_variable["prediction/b_fc4:0"]), tf.transpose(current_variable["prediction/W_fc4:0"]))
h_pool3_flat=tf.matmul((h_fc1_drop-current_variable["prediction/b_fc1:0"]), tf.transpose(current_variable["prediction/W_fc1:0"]))
h_conv22_=tf.reshape(h_pool3_flat, [1, 28, 1, 480 ])
#print h_pool21_
