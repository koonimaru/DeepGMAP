...
class Model(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fc1_param=int(math.ceil((self.data_length-self.conv1_filter+1)/6.0))
        self.prediction
        self.optimize
        self.error
        self.saver
    ...
    def prediction(self):
        ...
        def max_pool_13x1(x):
            return tf.nn.max_pool(x, ksize=[1, 6, 1, 1], strides=[1, 6, 1, 1], padding='SAME')
        ...
        def BiRNN(x):
            ...
            lstm_fw_cell = rnn.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(n_hidden, forget_bias=1.0), self.keep_prob2)
            ...
            lstm_bw_cell = rnn.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(n_hidden, forget_bias=1.0), self.keep_prob2)
