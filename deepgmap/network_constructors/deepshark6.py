...
class Model(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.hyper_param=kwargs["hyper_param"]
        
        self.fc1_param=0
        self.fc1_param+=self.data_length
        for k, v in self.hyper_param.items():
            if k.startswith("kernel_size"):
                self.fc1_param=self.fc1_param-v+1
            elif k.startswith("pooling"):
                self.fc1_param=int(math.ceil(self.fc1_param/float(v)))
        self.prediction
        self.optimize
        self.error
        self.saver
