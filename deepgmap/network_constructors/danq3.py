...
class Model(object):
    ...
    def prediction(self):
        ...
        def GridLSTM_network(x):
            ...
            frequency_skip=5
            feature_size=5
            num_frequency_blocks=(75-feature_size)/frequency_skip+1
            lstm_cell_fw = rnn.BidirectionalGridLSTMCell(n_hidden, 
                                                         forget_bias=1.0, 
                                                         use_peepholes=True,
                                                         ...
                                                         end_freqindex_list=[75])
            lstm_cell_bw = rnn.BidirectionalGridLSTMCell(n_hidden, forget_bias=1.0, use_peepholes=True,frequency_skip=frequency_skip,feature_size=feature_size, num_frequency_blocks=[num_frequency_blocks])
            ...
            lstm_cell_fw = rnn.DropoutWrapper(lstm_cell_fw, output_keep_prob=self.keep_prob2)
            lstm_cell_bw = rnn.DropoutWrapper(lstm_cell_bw, output_keep_prob=self.keep_prob2)
            #outputs, states = rnn.rnn(lstm_cell, x, dtype=tf.float32)
            outputs,_ = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw, lstm_cell_bw,x,
                                                dtype=tf.float32)
            return outputs
        ...
        def multiLSTM(x):
            #import pdb
            #pdb.set_trace()
            lstm_cell_fw = tf.nn.rnn_cell.LSTMCell(n_hidden, forget_bias=1.0, use_peepholes=True, state_is_tuple=True)
            lstm_cell_bw = tf.nn.rnn_cell.LSTMCell(n_hidden, forget_bias=1.0, use_peepholes=True, state_is_tuple=True)
            # lstm_cell = rnn_cell.GridLSTMCell(n_hidden, use_peepholes=True)
            lstm_cell_fw = rnn.MultiRNNCell([lstm_cell_fw] * 2, state_is_tuple=True)
            lstm_cell_bw = rnn.MultiRNNCell([lstm_cell_bw] * 2, state_is_tuple=True)
            lstm_cell_fw = rnn.DropoutWrapper(lstm_cell_fw, output_keep_prob=self.keep_prob2)
        ...
        def Grid2LSTM(x):
            #import pdb
            #pdb.set_trace()
            lstm_cell_fw = grid_rnn.Grid2LSTMCell(n_hidden, forget_bias=1.0, use_peepholes=True, state_is_tuple=True,tied=True)
            lstm_cell_bw = grid_rnn.Grid2LSTMCell(n_hidden, forget_bias=1.0, use_peepholes=True, state_is_tuple=True,tied=True)
            # lstm_cell = rnn_cell.GridLSTMCell(n_hidden, use_peepholes=True)
            #lstm_cell_fw = rnn.MultiRNNCell([lstm_cell_fw] * 2, state_is_tuple=True)
            #lstm_cell_bw = rnn.MultiRNNCell([lstm_cell_bw] * 2, state_is_tuple=True)
            lstm_cell_fw = rnn.DropoutWrapper(lstm_cell_fw, output_keep_prob=self.keep_prob2)
