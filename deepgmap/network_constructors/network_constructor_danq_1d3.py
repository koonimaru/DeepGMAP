...
class Model:
    ...
    def prediction(self):
        ...
        def BiRNN(x, weights, biases):
            ...
            lstm_fw_cell = tf.nn.rnn_cell.DropoutWrapper(cell=lstm_fw_cell, output_keep_prob=0.5)
            lstm_fw_cell = tf.nn.rnn_cell.MultiRNNCell(cells=[lstm_fw_cell] * 5, state_is_tuple=True)
            # Backward direction cell
            lstm_bw_cell = tf.nn.rnn_cell.LSTMCell(n_hidden, forget_bias=1.0,use_peepholes=True)
            lstm_bw_cell = tf.nn.rnn_cell.DropoutWrapper(cell=lstm_bw_cell, output_keep_prob=0.5)
            lstm_bw_cell = tf.nn.rnn_cell.MultiRNNCell(cells=[lstm_bw_cell] * 5, state_is_tuple=True)        
        ...
        weights = {
            ...
            'out': tf.Variable(tf.random_normal([2*n_hidden, 925]),name="W_rnn")}
            ...
            tf.log(
                tf.add(
                    ...
                    tf.clip_by_value(tf.multiply(tf.subtract(1.00,self.label), tf.subtract(1.00,self.prediction[1])),1e-10,1.0))
                                          ...
                                          reduction_indices=[1]))"""
        nll=tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(targets=self.label, logits=self.prediction[0],pos_weight=4.0))
